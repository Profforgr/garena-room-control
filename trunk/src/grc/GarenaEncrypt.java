...
public class GarenaEncrypt {
	...
	public void initAES() {
		Main.println("[GEncrypt] Initializing AES Keys...", GRCLog.SERVER);
	...
	public void initRSA() {
		Main.println("[GEncrypt] Initializing RSA Keys...", GRCLog.SERVER);
		BouncyCastleProvider bcp = new BouncyCastleProvider();
		Security.addProvider(bcp);

		Main.println("[GEncrypt] Reading private key in PEM format...", GRCLog.SERVER);
		try {
	...
	public String strFromBytes(byte[] input, String charset) {
		...
		return new String(input, 0, null_index, charset);
			...
			Main.println("[GEncrypt] " + charset + " is unsupported: " + e.getLocalizedMessage(), GRCLog.ERROR);
			return null;
		}
	}
	...
	public String strFromBytes16(byte[] input) throws IOException {
		...
		try {
			...
			Main.println("[GEncrypt] UnicodeLittleUnmarked is unsupported: " + e.getLocalizedMessage(), GRCLog.ERROR);
			return null;
		}
	}
	...
	public static byte[] externalAddress() {
		try {
			...
			Main.println("[GEncrypt] External IP address determined at " + ip.getHostAddress(), GRCLog.SERVER);
	...
	public static byte[] internalAddress() {
		try {
			...
			Main.println("[GEncrypt] Internal IP address determined at " + local_address.getHostAddress(), GRCLog.SERVER);
